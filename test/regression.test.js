...
test('회귀: install은 소스를 ~/.cccat/app으로 복사하고 훅이 그 안정 경로를 참조한다', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'cccat-reloc-'));
  const env = { ...process.env, CCCAT_HOME: path.join(tmp, '.cccat'), CCCAT_CLAUDE_DIR: path.join(tmp, '.claude') };
  fs.mkdirSync(env.CCCAT_CLAUDE_DIR, { recursive: true });
  fs.writeFileSync(path.join(env.CCCAT_CLAUDE_DIR, 'settings.json'), '{}');
  const CLI2 = path.join(__dirname, '..', 'bin', 'cccat.js');
  require('child_process').execFileSync(process.execPath, [CLI2, 'install'], { env, encoding: 'utf8' });
  // app 복사본 존재
  const appCli = path.join(env.CCCAT_HOME, 'app', 'bin', 'cccat.js');
  assert.ok(fs.existsSync(appCli), 'app/bin/cccat.js 복사됨');
  assert.ok(fs.existsSync(path.join(env.CCCAT_HOME, 'app', 'content', 'pack-core.json')), '콘텐츠 복사됨');
  assert.ok(fs.existsSync(path.join(env.CCCAT_HOME, 'app', 'content', 'i18n', 'ja.json')), 'i18n 복사됨');
  // 훅/런처가 app 경로 참조
  const launcher = fs.readFileSync(path.join(env.CCCAT_HOME, 'run.sh'), 'utf8');
  assert.ok(launcher.includes(appCli), '런처가 안정 app 경로 참조: ' + appCli);
  const s = JSON.parse(fs.readFileSync(path.join(env.CCCAT_CLAUDE_DIR, 'settings.json'), 'utf8'));
  const hookCmd = s.hooks.PreToolUse.find((e) => e.hooks[0].command.includes('cccat')).hooks[0].command;
  assert.ok(hookCmd.includes(path.join(env.CCCAT_HOME, 'run.sh')), '훅이 런처 경유');
  // app에서 실행해도 동작
  const out = require('child_process').execFileSync(process.execPath, [appCli, 'version'], { env, encoding: 'utf8' });
  assert.ok(out.includes('cccat v'));
  // uninstall이 app 정리
  require('child_process').execFileSync(process.execPath, [CLI2, 'uninstall'], { env, encoding: 'utf8' });
  assert.ok(!fs.existsSync(path.join(env.CCCAT_HOME, 'app')), 'uninstall이 app 제거');
});
