...
async function run() {
  ...
  const now = Date.now();
  let st = stateMod.load();
  let promoMessage = null;

  // 갱신이 필요한 경우에만 락을 잡고 read-modify-write (멀티 세션 안전).
  const recentlyActive = now - (st.activity_ts || 0) < 10 * 60 * 1000;
  const needRotate = !stateMod.isPaused(st, now) && recentlyActive &&
    (!st.current || now - st.current.shown_at > (config.rotate_sec || 30) * 3000);

  if (needRotate || st.promo_pending) {
    const path = require('path');
    const { withLock } = require('./store');
    withLock(path.join(require('./paths').ensureHome(), '.lock'), () => {
      st = stateMod.load(); // 락 안에서 다시 읽음
      let dirty = false;
      // hook이 아직 안 돌았거나 오래 멈춘 경우의 보조 회전.
