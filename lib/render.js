...
// 오늘 카운터 문구 (설명 언어별)
const COUNTER = {
  ko: { today: '오늘', unit: '개', review: '복습' },
  ja: { today: '今日', unit: '個', review: '復習' },
};
...
function render(st, config, opts = {}) {
  ...
  if (config.show_character) {
    ...
    head = `${C.cyan}${face}${C.reset} ${C.gray}${cat.label(activity, config.language)}${C.reset}`;
  }
  ...
  if (!compact && exampleLine) {
    const cnt = COUNTER[config.language] || COUNTER.ko;
    const counterText = st.today && st.today.shown
      ? ` · ${cnt.today} ${st.today.shown}${cnt.unit}${st.today.review ? ` (${cnt.review} ${st.today.review})` : ''}`
      : '';
