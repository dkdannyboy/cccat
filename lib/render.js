...
function blankOut(en) {
  ...
  if (core.length <= 2) return { text: en, word: null };
  const masked = w[0] + '＿'.repeat(Math.min(core.length - 2, 4)) + w[w.length - 1];
  words[idx] = masked;
...
function render(st, config, opts = {}) {
  ...
  if (displayWidth(line1) > width) {
    // 표현이 길면 상태 라벨 제거 후에도 넘치면 잘라냄
    if (config.show_character) {
      const face = cat.frame(activity, now, config.show_animation);
      line1 = `${C.cyan}${face}${C.reset}  ${exprPart}`;
    }
  }
  ...
  if (!compact && exampleLine) {
    const counter = st.today && st.today.shown
      ? ` ${C.gray}· 오늘 ${st.today.shown}개${st.today.review ? ` (복습 ${st.today.review})` : ''}${C.reset}`
      : '';
