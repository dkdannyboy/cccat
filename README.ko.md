...
[![languages](https://img.shields.io/badge/explanations-KO%20%7C%20JA-ff69b4.svg)](#설명-언어-한국어--日本語)
...
바로 쓰는 영어 표현을 한국어(또는 일본어) 뜻과 함께 띄워줍니다.

<table>
<tr>
<td align="center"><b>영어 표현 + 한국어 설명</b></td>
<td align="center"><b>영어 표현 + 일본어 설명</b></td>
</tr>
<tr>
<td><img src="assets/ko-statusline.png" alt="cccat 상태 표시줄 (한국어 설명)" width="100%"></td>
<td><img src="assets/ja-statusline.png" alt="cccat 상태 표시줄 (일본어 설명)" width="100%"></td>
</tr>
</table>

<sub>실제 cccat 상태 표시줄 렌더 엔진으로 만든 화면입니다 (`cccat lang ko` / `cccat lang ja`).
터미널에서는 상태에 따라 고양이 표정과 스피너가 실시간으로 움직이고, 상태 라벨(생각 중 / Git 작업 중 /
테스트 중 / 성공 …)과 설명이 모두 선택한 언어를 따릅니다. 실제 Claude Code TUI 캡처는
[docs/VERIFICATION.md](docs/VERIFICATION.md) 참고.</sub>
...
| 🇰🇷 **한국어·일본어 설명** | 개발 현장에서 바로 쓰는 영어 표현 + 자연스러운 한국어/일본어 뜻 + 예문 |
...
- Node.js **18 이상** (`npx`는 Node에 기본 포함)
- Claude Code (2.1.201 / macOS에서 검증됨. 자세한 내용은 [docs/COMPATIBILITY.md](docs/COMPATIBILITY.md))

## 설치

### 방법 1 — npx (가장 쉬움, 설치 불필요)

```sh
npx github:dkdannyboy/cccat install
```

클론도 전역 설치도 필요 없이 한 줄로 끝납니다. 나중에 `npx github:dkdannyboy/cccat uninstall`로 제거합니다.
...
cd cccat
node bin/cccat.js install     # 또는: npm install -g . && cccat install
```
...
기본 설명 언어는 한국어입니다. 일본어도 지원합니다(영어 표현 240여 개 전부 번역).

```sh
cccat lang ja     # 일본어 설명으로 전환
cccat lang ko     # 한국어로 되돌리기
cccat lang        # 현재 언어와 지원 목록 확인
```

일본어로 전환하면 뜻·예문·퀴즈 안내가 모두 일본어로 표시됩니다. 변경은 즉시 반영됩니다(재설치 불필요).
danielclass.com 안내는 한국어 사용자에게만 표시됩니다. 다른 언어 추가 방법은
...
| `cccat lang [code]` | 설명 언어 조회/변경 (`ko`, `ja`) |
...
| `language` | `ko` | 설명 언어 (`ko`, `ja`). `cccat lang <code>`로도 변경 가능 |
...
설치 즉시 검수된 240여 개 표현이 로컬에 들어 있어, 외부 API 없이도 바로 동작합니다. 실시간
...
버그 제보와 표현 제안 환영합니다. 이슈나 PR을 열어 주세요. 테스트는 `npm test`로
